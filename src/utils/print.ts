...
function generateReceiptHTML(order: Order): string {
  const rows = order.items.map((item) =>
    `<tr><td style="text-align:left;padding:2px 4px">${item.name}</td><td style="text-align:center;padding:2px 4px;width:30px">${item.quantity}</td><td style="text-align:right;padding:2px 4px;width:50px">$${item.price.toFixed(2)}</td><td style="text-align:right;padding:2px 4px;width:55px">$${(item.price * item.quantity).toFixed(2)}</td></tr>`
  ).join('');
  const cashLines = order.paymentMethod === 'Cash'
    ? `<div>Received: $${order.amountReceived.toFixed(2)}</div><div>Change: $${order.change.toFixed(2)}</div>`
    : '';
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Receipt ${order.invoiceNumber}</title><style>
    ...
    <div style="border-top:1px dashed #000;padding-top:4px;margin-bottom:6px">
      <div style="display:flex;justify-content:space-between;font-size:10px;padding:1px 0"><span>Subtotal:</span><span>$${order.subtotal.toFixed(2)}</span></div>
      <div style="display:flex;justify-content:space-between;font-size:10px;padding:1px 0;color:#15803d"><span>Discount:</span><span>-$${order.discount.toFixed(2)}</span></div>
      <div style="display:flex;justify-content:space-between;font-size:10px;padding:1px 0"><span>Tax (8%):</span><span>$${order.tax.toFixed(2)}</span></div>
      <div style="display:flex;justify-content:space-between;font-weight:bold;font-size:14px;margin-top:4px;padding-top:4px;border-top:1px solid #000"><span>TOTAL:</span><span>$${order.grandTotal.toFixed(2)}</span></div>
    </div>
    <div style="margin-bottom:6px;padding-bottom:6px;border-bottom:1px dashed #000;font-size:10px">
